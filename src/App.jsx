...
import { MediaKitForm } from './MediaKitForm.jsx';
...
function Coverage({ lang, detailed = false }) {
  const c = copy[lang];
  return <section className="section coverage-section">{detailed ? <><p className="eyebrow">{lang === 'en' ? 'Portugal / Coverage' : 'Portugal / Cobertura'}</p><h1>{c.coverageTitle}</h1></> : <SectionTitle label={lang === 'en' ? 'Portugal / Coverage' : 'Portugal / Cobertura'}>{c.coverageTitle}</SectionTitle>}<p className="large-copy section-intro">{c.coverageBody}</p><div className="city-grid">{['Lisboa', 'Porto', 'Algarve'].map((city, i) => <a key={city} href={`${pageLink('contact', lang)}?city=${city}`} className="city-card">{i < 2 && <span className="city-card-media"><img src={`/assets/cartise-${i === 0 ? "lisboa" : "porto"}.jpg`} alt={lang === "en" ? `Illustrative AI photograph of ${city}` : `Fotografia ilustrativa de ${city}, gerada com IA`} width="1536" height="1024" loading="lazy" /></span>}<span className="city-index">PT / 0{i + 1}</span><h3>{lang === 'en' && city === 'Lisboa' ? 'Lisbon' : city}</h3><span>{i < 2 ? (lang === 'en' ? 'Active network · enquire about dates' : 'Rede ativa · consultar datas') : c.availability}<ArrowUpRight size={18} /></span></a>)}</div>{!detailed && <a className="text-button" href={pageLink('coverage', lang)}>{c.coverageLink}<ArrowUpRight size={16} /></a>}</section>;
}
...
export function App({ pathname = '/' }) {
  ...
  return <div className="site-shell" id="top">
    ...
    <main id="main">
      ...
      {key === 'mediaKit' && <><section className="media-kit-hero section"><div><p className="eyebrow">Cartise / Media kit</p><h1>{en ? 'Everything you need to introduce Cartise.' : 'Tudo o que precisa para apresentar a Cartise.'}</h1><p className="large-copy">{en ? 'Request our media kit for a clear introduction to digital advertising inside TVDE vehicles in Portugal.' : 'Peça o nosso media kit para conhecer a publicidade digital dentro de veículos TVDE em Portugal.'}</p><p className="media-kit-note">{en ? 'We will send the download link to your work email.' : 'Enviaremos o link de download para o seu email profissional.'}</p></div><div className="media-kit-card"><p className="eyebrow">{en ? 'Request access' : 'Pedir acesso'}</p><h2>{en ? 'Get the media kit by email.' : 'Receba o media kit por email.'}</h2><MediaKitForm lang={lang} /></div></section><section className="media-kit-assurance section"><p>{en ? 'For campaign proposals, availability and commercial conditions, talk directly with our team.' : 'Para propostas de campanha, disponibilidade e condições comerciais, fale diretamente com a nossa equipa.'}</p><a className="text-button" href={link('contact')}>{c.contact}<ArrowUpRight size={16} /></a></section></>}
    ...
    <footer className="footer"><div className="footer-brand"><a className="brand-mark" href={link('home')}>CARTISE</a><p>{c.signature}</p><span>{c.footerCompany}</span></div><nav aria-label={en ? 'Footer navigation' : 'Navegação de rodapé'}>{[['solution', c.nav[0]], ['formats', c.nav[1]], ['coverage', c.nav[2]], ['fleets', c.nav[3]], ['mediaKit', en ? 'Media kit' : 'Media kit'], ['privacy', c.privacy]].map(([key, label]) => <a href={link(key)} key={key}>{label}</a>)}</nav><div className="footer-contact"><a href="mailto:hello@cartise.pt">hello@cartise.pt ↗</a><a className="footer-phone" href="tel:+351928406723">{c.phone} ↗</a><a href={link('contact')}>{c.contact}</a></div></footer>
